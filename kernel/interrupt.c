...
static void pic_init(void) {
   ...
   outb (PIC_M_DATA, 0xf8);    //将0xfe改为0xfe后暂时只支持键盘中断,f8设置后响应从片上的中断
   outb (PIC_S_DATA, 0xbf);    //变为bf后接受硬盘的中断
...
static void exception_init(void) {			    // 完成一般中断处理函数注册及异常名称注册
   int i;
   for (i = 0; i < IDT_DESC_CNT; i++) {
/* idt_table数组中的函数是在进入中断后根据中断向量号调用的见kernel/kernel.S的call [idt_table + %1*4] */
