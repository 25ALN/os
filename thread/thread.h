...
typedef void thread_func(void*); //函数声明 
...
struct task_struct
{
    ...
    struct list_elem all_list_tag;		      //总队列的连接节点s
    
    uint32_t* pgdir;				      //进程自己页表的虚拟地址 线程没有
    struct virtual_addr userprog_vaddr;   //用户的虚拟地址空间
    struct mem_block_desc u_block_desc[DESC_CNT];
    uint32_t cwd_inode_nr;			      //工作目录inode编号
    uint32_t stack_magic;			      //越界检查  因为pcb上面的就是我们要用的栈了 到时候还要越界检查
    pid_t parent_pid;                              // 父进程的pid
    int8_t exit_status;                             // 进程退出状态
};
